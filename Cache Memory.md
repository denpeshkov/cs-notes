...
The cache stores and loads the whole **cache line** from/to lower levels. See: [[Memory Hierarchy and Locality]]

---

See [[Snoop-Based Multiprocessor Design]] for implementation details
...
Write is done synchronously both to the cache and to the [[Memory Hierarchy and Locality|next lower level]]

Inefficient because of a very high bandwidth requirement

## Write-Back

Write is initially done only to the cache. Write to the [[Memory Hierarchy and Locality|next lower level]] is postponed for as long as possible and done only when the line is **evicted** from the cache by the replacement algorithm
...
Data at the missed-write location is not loaded into the cache and is written directly to the [[Memory Hierarchy and Locality|next lower level]]
...
Instructions use [[Virtual Memory|virtual addresses]], caches can use virtual or physical addresses for the index and tag

## Physically Indexed, Physically Tagged (PIPT)

Use the physical address for both the index and the tag

Simple but slow because the need to translate to a virtual address, which may involve a [[Virtual Memory|TLB]] miss and [[Random Access Memory|RAM]] access

## Virtually Indexed, Virtually Tagged (VIVT)

Use the [[Virtual Memory|virtual address]] for both the index and the tag
...
Use the [[Virtual Memory|virtual address]] for the index and the physical address in the tag

The advantage over PIPT is lower latency, as the cache line can be looked up in parallel with the [[Virtual Memory|TLB]] translation, but the tag cannot be compared until the physical address is available
...
Misses due to [[Cache Coherency|invalidations]] when using [[Cache Coherency|invalidation cache coherency protocol]]
